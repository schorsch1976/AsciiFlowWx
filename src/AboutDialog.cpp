...
AboutDialog::AboutDialog()
	: wxDialog(NULL, -1, "About", wxDefaultPosition, wxSize(250, 230))
{
	wxBoxSizer *mainsizer = new wxBoxSizer(wxVERTICAL);

	// the logo box
	wxBoxSizer *logo_box = new wxBoxSizer(wxHORIZONTAL);
	mainsizer->Add(logo_box, 0, wxALIGN_CENTER);
	wxStaticText *logo = new wxStaticText(this, -1, ascii);
...
{
	wxNotebookPage *page = new wxNotebookPage(parent, 0, wxDefaultPosition,
											  wxDefaultSize, wxEXPAND | wxALL);

	wxBoxSizer *padder = new wxBoxSizer(wxVERTICAL);
	wxGridSizer *sizer = new wxGridSizer(5, 2, 5, 5);
	...
	sizer->Add(AddLink(page, "https://github.com/schorsch1976/AsciiFlowWx/blob/"
							 "master/LICENCE.txt"),
			   0, wxEXPAND);
